...
class RptsiswacabangEDCController extends ControllerBase
{
    ...
    public function initialize() {
        $this->tag->setTitle('Laporan EDC');
        parent::initialize();
        if ($this->session->has('auth')) {
            $this->auth = $this->session->get('auth');
        }
        $this->view->rpt_title = 'Laporan transaksi EDC';
    }
 ...
 public function viewAction() {
        ...
        if (!$this->request->isPost()) {
            return $this->forward('rptsiswacabangedc/index');
        }
	...
	//	}
        ...
        $sql = "select VirtualAccount,MD, NamaSiswa,Cabang,sum(JumlahTotal)as JumlahTotal, 
(select sum(transaksibank.Nominal) as Uang_masuk from transaksibank where transaksibank.Siswa = cte.RecID and 
tahunajaran=$tahun and KodeCabang=cte.KodeAreaCabang and transaksibank.auth_cd is not null)as Uang_masuk,
(select sum(transaksibank.Nominal) as Uang_masuk from transaksibank where transaksibank.Siswa = cte.RecID and 
tahunajaran=$tahun and KodeCabang=cte.KodeAreaCabang and transaksibank.auth_cd is null)as Uang_nonedc,
...
(select siswa.VirtualAccount,Siswa.RecId,Siswa.MD,areacabang.kodeareacabang, siswa.NamaSiswa,siswa.cabang, 
               (pembayaran.JumlahTotal-pembayarandetail.Jumlah) as
				...
				join transaksibank on siswa.recid=transaksibank.siswa
                Join areacabang ON areacabang.RecID = siswa.cabang
               Where program.tahunajaran = $tahun and siswa.cabang =$cabang
			   and pembayarandetail.pembayaranuntuk='Pendaftaran' and transaksibank.auth_cd is not null

...
class PembayaranfranchiseController extends ControllerBase
{
    protected $auth;
    ...
    public function initialize() {
        ...
        $this->view->rpt_title = 'Input Data Pembayaran Franchise';
    }
    ...
    public function searchAction()
    {
		...
		$sql1 = "select top 1 a.RecID, REPLACE (areacabang.KodeAreaCabang, ' ', '' ) as KodeAreaCabang,
            ...
            pembayaranfranchisee.TglMou, pembayaranfranchisee.keterangan ,areacabang.TanggalBerlaku,FranchiseFeeId,
    ...
    public function saveAction()
    {
        ...
        $insert = "insert into pembayaranfranchisee (RecID,KodeCabang, NilaiFranchisee, TglMOU, Total, TahunMulai, TahunAkhir,Keterangan, FranchiseFeeID,Diskon, AwalKontrak, AkhirKontrak)
		values((SELECT ISNULL(MAX(RecID) + 1, 0) FROM pembayaranfranchisee), '$kodeAreaCabangtrim','$NilaiFranchisee','$TglMou','$total','$tahunBerlaku','$tahunBerakhir','$Keterangan','$FFID','$Diskon','$berlaku2','$berakhir2')"; 
		$query = $this->getDI()->getShared('db')->query($insert);
        $query->setFetchMode(Phalcon\Db::FETCH_OBJ);
	
		
	
		$update = "update areacabang set TanggalBerlaku = '$berlaku2', TanggalBerakhir = '$berakhir2' where KodeAreaCabang = '$kodeAreaCabangtrim'";
		
		$query1 = $this->getDI()->getShared('db')->query($update);
        $query1->setFetchMode(Phalcon\Db::FETCH_OBJ);
		
		$this->flash->success("cabang baru sudah berhasil diinputkan");

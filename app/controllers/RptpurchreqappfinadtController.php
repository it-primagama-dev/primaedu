...
class RptpurchreqappfinadtController extends ControllerBase {
    ...
    public function viewAction() {
        if (!$this->request->isPost()) {
            return $this->forward('Rptpurchreqappfinadt/index');
        }
        ...
        $purchreq = Purchreqheader::query()
                //->columns('d.PurchReqId,d.Status,d.ApprovalDate,Purchreqheader.*')
        ...
        if ($tgl) {
            $purchreq->andWhere('d.ApprovalDate >= :tgl:', ['tgl' => $tgl]);
        }
        if ($datefrom) {
        ...
        $this->view->periodeakhir = date('d/m/Y', strtotime($dateto));

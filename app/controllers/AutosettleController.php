...
FROM transaksibank tx JOIN (
    ...
    WHERE tx.RefRecID IS NULL  AND tx.KodeCabang='?0' AND pd.status='0'
...
SQL;
    ...
    public function prosesAction() {
        ...
        $cabang = $this->auth['kodeareacabang'];
       $sql = str_replace('?0', $cabang, $this->sql_0);
	  
        // Update Transaksibank
        if (!$this->db->execute($sql)) {
            $this->db->rollback();
            $this->flash->error('Gagal melakukan');
        ...
        if (!$this->db->execute($this->sql_2)) {
            $this->db->rollback();
            $this->flash->error('Gagal melakukan Auto ');
            return $this->forward('autosettle/index');
        }
		
		  if (!$this->db->execute($this->sql_1)) {
            $this->db->rollback();
            $this->flash->error('Gagal melakukan update tanggal ');
            return $this->forward('autosettle/index');
        }
        //Check Affected Rows

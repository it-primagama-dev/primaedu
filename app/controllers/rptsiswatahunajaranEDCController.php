...
class RptsiswatahunajaranEDCController extends ControllerBase
{
    ...
    public function initialize() {
        $this->tag->setTitle('Laporan Transaksi Bank EDC');
        parent::initialize();
 ...
 public function viewAction() {
        ...
        if (!$this->request->isPost()) {
            return $this->forward('rptsiswatahunajaranedc/index');
        }

        $tahun = $this->request->getPost('tahun');
        $cabang = $this->request->getPost('Cabang');
		
        $sql = "select  siswa.NamaSiswa as siswa,siswa.VirtualAccount,transaksibank.NamaBank,transaksibank.TanggalTransaksi,transaksibank.TanggalImport, transaksibank.WaktuTransaksi,transaksibank.NoVA,transaksibank.Nominal, tahunajaran.Description, transaksibank.gros_amt , transaksibank.seq,areacabang.KodeAreaCabang from transaksibank 
				...
				where  transaksibank.auth_cd is not null and areacabang.RecID='$cabang' and isnull(tahunajaran.RecID,1) = '$tahun' and transaksibank.Nominal > '2500' order by NoVA
                ";

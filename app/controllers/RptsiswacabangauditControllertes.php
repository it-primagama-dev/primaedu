<?php 
set_time_limit(120); 
class RptsiswacabangauditController extends ControllerBase
{
 ...
 public function viewAction() {
        ...
        if (!$this->request->isPost()) {
            return $this->forward('Rptsiswacabangaudit/index');
        }
        ...
        $sql="select VirtualAccount,CONVERT(VARCHAR(30),CreatedAt,105) as CreatedAt,MD,NamaJenjang,NamaSiswa,Cabang,sum(JumlahTotal)as JumlahTotal,TanggalPembayaran as TanggalBayarInput,
                (select sum(transaksibank.Nominal) as Uang_masuk from transaksibank where transaksibank.Siswa = cte.RecID and 
                tahunajaran='$tahun' and KodeCabang=cte.KodeAreaCabang)as Uang_masuk,
                +(select sum(jumlah) as uang from pembayarandetail join pembayaran on pembayaran.RecID=pembayarandetail.Pembayaran
join programsiswa on programsiswa.RecID=pembayaran.ProgramSiswa
...
Join areacabang ON areacabang.RecID = siswa.cabang where programsiswa.Siswa
                ...
                (select sum(jumlah) as uang from pembayarandetail join pembayaran on pembayaran.RecID=pembayarandetail.Pembayaran
                                join programsiswa on programsiswa.RecID=pembayaran.ProgramSiswa
                                join program on program.RecID=programsiswa.program where programsiswa.Siswa
                                =cte.RecID and PembayaranUntuk='Bimbingan' and program.tahunajaran = '$tahun') as uanginputadmin, 
                (select Jumlah as uang from pembayarandetail join pembayaran on pembayaran.RecID=pembayarandetail.Pembayaran
                                join programsiswa on programsiswa.RecID=pembayaran.ProgramSiswa
                                join program on program.RecID=programsiswa.program where programsiswa.Siswa
                                =cte.RecID and PembayaranUntuk='Pendaftaran' and program.tahunajaran = '$tahun') as uangpendaftaran
                                from
                (select siswa.VirtualAccount,Siswa.CreatedAt,Siswa.RecId,Siswa.MD,areacabang.kodeareacabang,
                 siswa.NamaSiswa,siswa.cabang, jenjang.NamaJenjang,pembayarandetail.TanggalPembayaran,
                               (pembayaran.JumlahTotal-pembayarandetail.Jumlah) as
                JumlahTotal From Siswa
                Join jenjang ON siswa.Jenjang = jenjang.KodeJenjang
                Join programsiswa ON siswa.RecID = programsiswa.siswa
               ...
               Where program.tahunajaran = '$tahun' and siswa.cabang ='$cabang'
               and pembayarandetail.pembayaranuntuk='Pendaftaran'
                group by siswa.VirtualAccount,Siswa.CreatedAt,Siswa.MD,pembayaran.JumlahTotal, siswa.NamaSiswa,
                 program.NamaProgram, siswa.cabang, siswa.RecID,Jenjang.NamaJenjang,pembayarandetail.TanggalPembayaran,
                tahunajaran.Description,pembayarandetail.Jumlah,areacabang.kodeareacabang) as cte
                group by VirtualAccount,CreatedAt,MD,NamaSiswa,Cabang,RecID,kodeareacabang,NamaJenjang,TanggalPembayaran";

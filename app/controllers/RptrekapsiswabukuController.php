...
class rptrekapsiswabukuController extends ControllerBase
{
...
select area.NamaAreaCabang as Area, b.KodeAreaCabang as KodeCabang, b.NamaAreaCabang as NamaCabang, 
a.PurchReqDate,   
TotalQty = (select isnull ( sum(Qty),0) from purchreqline where purchreqheader = a.RecId),
TotalSiswa =  (select count(RecId) from siswa where cabang = a.Cabang)
from purchreqheader a
join areacabang b on b.RecID =a.Cabang
join areacabang area on area.KodeAreaCabang = b.Area
%2
order by b.KodeAreaCabang asc
SQL;


    public function initialize() {
        $this->tag->setTitle('Laporan Jumlah Siswa dan Buku');
        parent::initialize();
    ...
    public function viewAction()
    {
        if (!$this->request->isPost()) {
            return $this->forward('rptrekapsiswabuku/index');
        }

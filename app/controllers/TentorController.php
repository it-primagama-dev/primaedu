...
class TentorController extends ControllerBase {
    ...
    public function searchAction() {
        ...
        if ($this->request->isPost()) {
            ...
            if($this->auth['areaparent']) {
                ...
                $query->andWhere("Pendidikan is not null");
                $query->andWhere("Grade is null");
            }
    ...
    public function viewAction() {
       ...
       /* $ismart = Ismart::find($parameters);
            if($this->auth['areaparent']) {
                $ismart->andWhere("Cabang = :cabang:", ["cabang" => $this->auth['areacabang']]);
                $ismart->andWhere("Pendidikan is not null");
            }
            $this->persistent->parameters = $ismart->getParams();

        echo $ismart;*/
        $ismart = Ismart::query()
                ->where("Cabang = :cabang:", ["cabang" => $this->auth['areacabang']])
                ->andWhere('Pendidikan is not null')
                ->execute();
    ...
    public function editAction($RecID) {
        ...
        if (!$this->request->isPost()) {
            ...
            $this->tag->setDefault("Jurusan", $ismart->Jurusan);
            $this->tag->setDefault("Pendidikan", $ismart->Pendidikan);
    ...
    public function createAction() {
        ...
        if (strlen($ismart->BidangStudi) == 0 || strlen($ismart->TanggalGabung) == 0) {
            $field = strlen($ismart->BidangStudi) == 0 ? "Bidang Studi" : "Tanggal Gabung";

        ...
        <td colspan="7">
            <table class="table bordered hovered table2excel" style="width: 140%; border-collapse: collapse">
                <thead>
                    <tr>
                        <th width="4%">#</th>
                        <th width="6%">No. Induk Siswa</th>
                        <th width="15%">Nama Siswa</th>
                        <th width="7%">Jenjang</th>
                        <th width="9%">Total Biaya</th>
                        <th width="9%">Total Pembayaran</th>
                         <th width="10%">Total Discount</th>
                        <th width="10%">Total Hutang</th>
                         <th width="5%">Mengundurkan diri</th>
                         <th width="8%">Tanggal Input Di System</th>
                         <th width="8%">Tanggal Input Pendaftaran</th>
                         <th width="20%">Jumlah Biaya Bimbingan Yang Di Input</th>
                    ...
                    {% if result is not empty %}
                        {% for list in result %}
                            ...
                            <tr class="text-center">
                                 ...
                                 <td>{{ loop.index }}</td>
                                <td><a href="{{ url('Rptsiswacabangauditdetail/view/'~list.VirtualAccount)}}{{ rpt_cabangid }} " target="_blank">{{ list.VirtualAccount }}</a></td>
                                ...
                                <td class="text-center">Rp {{ sisa|number_format(0,',','.') }}</td>
                                    ...
                                    <td class="text-center">{{ list.CreatedAt}}</td>
                                    <td class="text-center">{{ list.TanggalBayarInput}}</td>
                                    <td class="text-center"><a href="{{ url('Rptsiswacabangauditdetail2/view/'~list.VirtualAccount)}}{{rpt_tahunid }}{{ rpt_cabangid }} " target="_blank">Rp {{ list.uanginputadmin|number_format(0,',','.') }}</a>{% set sumuanginputadmin+=list.uanginputadmin %}</td>
                                    <td class="text-center">Rp {{ list.uangpendaftaran|number_format(0,',','.') }}
                                    </td>
                                {% else  %}
                                     <td bgcolor='red'>{{ loop.index }}</td>
                                <td bgcolor='red'><a href="{{ url('Rptsiswacabangauditdetail/view/'~list.VirtualAccount)}}{{ rpt_cabangid }} " target="_blank">{{ list.VirtualAccount }}</a></td>
                                ...
                                <td class="text-left" bgcolor='red'>{{ list.CreatedAt}}</td>
                                <td class="text-center" bgcolor='red'>{{ list.TanggalBayarInput}}</td>
                                <td class="text-center" bgcolor='red'><a href="{{ url('Rptsiswacabangauditdetail2/view/'~list.VirtualAccount)}}{{rpt_tahunid }} {{ rpt_cabangid }}" target="_blank">Rp {{ list.uanginputadmin|number_format(0,',','.') }}</a>{% set sumuanginputadmin+=list.uanginputadmin %}</td>
                                <td class="text-center" bgcolor='red'>Rp {{ list.uangpendaftaran|number_format(0,',','.') }}
                                </td>
                                 {% endif %}
                                    
                                {% set hutangg=biaya-pembayaran-discount %}
                                {% set totsis=totsis+1 %}
                                {% set sumpendaftaran+=list.uangpendaftaran %}
                        {% endfor %}
                    {% else %}
            ...
            {% endif %}         
                    ...
                    <tr>
                        ...
                        <th  align='center'> </th>
                        <th  align='center'> </th>
                        <th  align='center'>Rp {{sumuanginputadmin|number_format(0,',','.')}}</th>
                        <th  align='center'>Rp {{sumpendaftaran|number_format(0,',','.')}}</th>
                    <tr>

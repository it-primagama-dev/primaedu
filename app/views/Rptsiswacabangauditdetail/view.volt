...
<table class="table bordered hovered table2excel" style="table-layout: fixed; border-collapse: collapse">
    <thead>
        <tr>
            ...
            <th width="220px" rowspan="2">Biaya Bimbingan  </th>
            <th width="220px" rowspan="2">Jumlah Yang Dibayarakan</th>
        </tr>
    </thead>
    <tbody>
        {% if result is not empty %}
            {% for list in result %}
                <tr>
                    ...
                    <td class="text-center">{{ (list.BiayaBimbingan)|number_format(0,',','.') }}</td>
                    <td class="text-center">{{ (list.Nominal)|number_format(0,',','.') }}{% set sumnom += list.Nominal %}</td>
...
<div class="place-right">
    {{ link_to("Rptsiswacabangauditdetail/view", "First", 'class':'button') }}
    {{ link_to("Rptsiswacabangauditdetail/view?page="~page.before, "Previous", 'class':'button') }}
    {{ link_to("Rptsiswacabangauditdetail/view?page="~page.next, "Next", 'class':'button') }}
    {{ link_to("Rptsiswacabangauditdetail/view?page="~page.last, "Last", 'class':'button') }}
</div>
{% endif %}
#}
<script>

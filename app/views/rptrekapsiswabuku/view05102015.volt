...
<table class="table bordered hovered table2excel" style="table-layout: fixed; border-collapse: collapse">
    <thead>
        <tr>
            ...
            <th width="300px">Tanggal Pembelian Buku</th>
        </tr>
    ...
    <tbody>
                    ...
                    {% if result is not empty %}
                        {% for list in result %}
                            ...
                            {% if currKodeCabang != lastKodeCabang and not loop.first %}
                                <tr class="text-right" style="background: #f0f0f0; font-weight: bold">
                                    <td colspan="4" align='right'>Sub Total</td>
                                    <td align='center'>{{ subD2|number_format(0,',','.') }}</td>
									<td>&nbsp;</td>
                                </tr>
                                {% set subD1 = 0 %}{% set subD2 = 0 %}{% set subD3 = 0 %}
                            {% endif %}
                            {% set subD1 += list.TotalSiswa %}
                            {% set subD2 += list.TotalQty %}

                <tr>
                    <td>{{ loop.index }}</td>
                    <td>{{ list.Area }}</td>
                    <td>{{ list.KodeCabang|trim~' / '~list.NamaCabang }}</td>
                    <td class="text-center">{{ list.TotalSiswa|number_format(0,',','.') }}</td>
                    <td class="text-center">{{ list.TotalQty|number_format(0,',','.') }}</td>

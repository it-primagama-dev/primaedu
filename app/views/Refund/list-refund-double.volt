...
{{ form("refund/ubahfinance", "method":"post","enctype": "multipart/form-data","id":"approved4") }}
<div class="grid fluid no-margin">
    <div class="row">
        <div class="col-md-12">
            {% if username == 'devyana' OR username == 'kitty' OR username == 'ukky' OR username == 'endra' %}
            <div class="btn-group place-right">
                <div class="input-control select" data-role="input-control" style="margin-top: 3px; margin-left: 7px;">
                    {{ select_static("onchange":"ganti3('this')","ubahstatus3","name":"ubahstatus", ["":"Ubah Status","Approved" : "Approved", "Rejected" : "Rejected"]) }}
                </div>
            </div>
            {% endif %}
            <table id="dt-refund-double" class="table table-bordered table-striped" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th>Jumlah Nominal Salah</th>
                        <th>Jumlah Nominal Benar</th>
                ...
                <tbody>
                    {% for key,rows in list_refund_double %}
                    <tr>
                        ...
                        <td class="Idr">{{ rows.LebihNominal }}</td>
                        ...
                        <td width="2%" style="text-align: center;">
                            {% if username == 'kitty' AND rows.TrackFinance == 'Pending' 
                            ...
                            {% endif %}
                            {% if username == 'endra' AND rows.TrackGM == 'Pending' AND rows.TrackFinance == 'Approved' 
                                OR username == 'ukky' AND rows.TrackGM == 'Pending' AND rows.TrackFinance == 'Approved'%}
                                <input type="checkbox" name="RecID[]" value="{{ rows.RecID }}">
                            {% endif %}
                        </td>
                    </tr>
...
<script>
function ganti3(elem){
    var ubahstatus = document.getElementById("ubahstatus3");
    $(document).ready(function() {
        if(ubahstatus.value === "Rejected") {
            $('#modalSuccess4').modal('show');
        } else if(ubahstatus.value === "Approved") {
            $("form#approved4").submit();
            $('.loader').show();
        }
    });
}   
...
<div class="modal fade modal-success" id="modalSuccess4">

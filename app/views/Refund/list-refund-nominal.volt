...
{{ form("refund/ubahfinance", "method":"post","enctype": "multipart/form-data","id":"approved2") }}
        ...
        <div class="col-md-12">
            ...
            <div class="btn-group place-right">
                <div class="input-control select" data-role="input-control" style="margin-top: 3px; margin-left: 7px;">
                    {{ select_static("onchange":"ganti2('this')","ubahstatus2","name":"ubahstatus", ["":"Ubah Status","Approved" : "Approved", "Rejected" : "Rejected"]) }}
                </div>
            </div>
            {% endif %}
            <table id="dt-refund-nominal" class="table table-bordered table-striped" style="width:100%">
                <thead>
                    <tr>
                ...
                <tbody>
                    {% for key,rows in list_refund_nominal %}
                    <tr>
...
<script>
function ganti2(elem){
    var ubahstatus = document.getElementById("ubahstatus2");
    $(document).ready(function() {
        if(ubahstatus.value === "Rejected") {
            $('#modalSuccess2').modal('show');
        } else if(ubahstatus.value === "Approved") {
            $("form#approved2").submit();
            $('.loader').show();
        }
    });
}   
...
<div class="modal fade modal-success" id="modalSuccess2">

<h1>
    {{ link_to("pbview/index", '<i class="icon-arrow-left-3 fg-darker smaller"></i>') }}
    Inquiry Pembayaran
    <small>View</small>
</h1>
        ...
        <tbody>
            {% for list in pembayaran %}
                ...
                <tr>
                    ...
                    <td class="text-center" data-pembayaran="{{ list.Pembayaran }}">{{ link_to("pbview/view#", 'Detail') }}</td>
                </tr>
            {% endfor %}
        </tbody>
    </table>
{% endif %}
{{ form("pbview/detail", "method":"post") }}
    {{ hidden_field("RecID") }}
    {{ hidden_field("Cabang") }}
    {{ hidden_field("Siswa") }}
{{ end_form() }}

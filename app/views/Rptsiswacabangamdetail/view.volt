...
<table class="table bordered hovered table2excel" style="table-layout: fixed; border-collapse: collapse">
    <thead>
        <tr>
            ...
            <th width="220px" rowspan="2">Pembayaran </th>
            <th width="220px" rowspan="2">Jumlah Yang Dibayarakan</th>
            <th width="220px" rowspan="2">Sisa Pembayaran</th>
        </tr>
    </thead>
    <tbody>
        {% if result is not empty %}
            {% for list in result %}
                <tr>
                    ...
                    <td class="text-center">{{ (list.PembayaranUntuk) }}</td>
                    <td class="text-center">{{ (list.Jumlah)|number_format(0,',','.') }}</td>
                    <td class="text-center">{{ (list.SisaPembayaran)|number_format(0,',','.') }}</td>

...
$(document).ready(function(){
    ...
    if(kode === '1486736823') {
        ...
        var url = "{{ url('registrasi/program2/') }}" + $("#Jenjang option:selected").val();
    }
    ...
    $(".tab-control").tabcontrol().bind("tabcontrolchange", function(event, frame){
        ...
        if(id === "_tab2") {
            ...
            $.getJSON(urlHarga).done(function (data) {
                ...
                $('#BiayaPendaftaran2').empty();
                if (data.status === "OK") {
                    $('#BiayaPendaftaran').val(data.listData[0].hargapendaftaran).autoNumeric('update');
                    $('#BiayaPendaftaran2').val(data.listData[0].hargapendaftaran).autoNumeric('update');
                }
            });
            setpaymethod();
        }
    ...
    $('#btn-submit').on('click', function () {
        ...
        var hargapas = document.getElementById('BiayaPendaftaran2').value;
        var harga = document.getElementById('BiayaPendaftaran').value;
            
            
            if (harga > hargapas){
                  alert('Harga Pendaftaran Melebihi Batas Maksimal !');
                  $("#myModal").modal('hide');
                $('#BiayaPendaftaran').focus().parent().addClass('error-state');$(".loader").hide();return;
            }
            ...
            if(asal===''||asal===null){
                alert("Asal Sekolah wajib di isi");
                $("#myModal").modal('hide');

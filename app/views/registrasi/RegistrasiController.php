...
class RegistrasiController extends ControllerBase {
    ...
    public function indexAction() {
        ...
        $sql = "SELECT ISNULL(Cilegal,0) as cilegal, ISNULL(Cilegal18,0) as cilegal18 from Areacabang where RecID ='$cabang'";
...
//            $this->session->set("noIndukSiswa", $noIndukSiswa);
    ...
    public function programAction($RecID) {
        ...
        $records = [];
        $program= Program::find(["Jenjang ='$RecID' AND tahunajaran='5' AND NamaProgram NOT LIKE '%2018/2019%'"]);    
    ...
    public function program2Action($RecID) {
        ...
        $response->setContentType('application/json');
        $records = [];
        $program= Program::find(["tahunajaran='4' and Jenjang ='$RecID' AND NamaProgram NOT LIKE '%2017/2018%' "]);       
    if (!count($program)) {
            $response->setJsonContent(array('status' => 'Not Found'));
        } else {
            foreach ($program as $record) {
    ...
    public function hargaAction($programid) {
        $this->view->disable();
        $cabang = $this->auth['areacabang'];
        $sql = "SELECT Area from Areacabang where RecID ='$cabang'";
        $query = $this->getDI()->getShared('db')->query($sql);
        $query->setFetchMode(Phalcon\Db::FETCH_OBJ);
        $this->view->result = $query->fetchAll($query);
    ...
    public function prosesAction() {
        ...
        if ($noIndukSiswa) {
            ...
            $siswa = new Siswa();
            $siswa->setTransaction($transact);
            $program1 = $this->request->getPost("Program");
            ...
            $kodeVA = Virtualaccountlist::findFirst(array(
               ...
               // echo ""
                ...
                $kodeVA = VirtualaccountlistNew::findFirst(array(
                "KodeCabang = :cabang: AND (IsUsed = 0 OR IsUsed = NULL)",
                "bind" => array("cabang" => $this->auth['kodeareacabang'])
            ));
              //  echo "VA baru";
...
//die();
        ...
        if ($jumlahBayar > 0) {
            ...
            if ($metode != FALSE) {
                ...
                if ($param[0] == 'VA') {
                    ...
                    $pembayarandetail2->NoReferensi .= $this->request->getPost("NoReferensi1");
                    $pembayarandetail2->NoReferensi .= $this->request->getPost("NoReferensi2");
					$pembayarandetail2->CardNo = $this->request->getPost("CardNo");
					$pembayarandetail2->AuthCd = $this->request->getPost("AuthCd");
                } elseif ($param[0] = 'KP') {
    ...
    public function siswabaruAction($kode) {
        //TODO
        $cabang = $this->auth['areacabang'];
        $sql = "SELECT rtrim(area) as area from Areacabang where RecID ='$cabang'";
        $query = $this->getDI()->getShared('db')->query($sql);
        $query->setFetchMode(Phalcon\Db::FETCH_OBJ);
        $this->view->result = $query->fetchAll($query);
       if ($this->request->isPost()) {
    ...
    private function validate() {
		...
		if ($metode != FALSE) {
			...
			if ($param[1] == "DebitBCA") {
				...
				if (empty($this->request->getPost("CardNo")) || is_null($this->request->getPost("CardNo"))) {
					$this->flash->error('Please Insert Number Card And Fill Appr. Code');
					return FALSE;
        ...
        if (is_null($this->request->getPost('NoReferensi', 'int'))) {
            $this->flash->error('No Referensi harus diisi');
            return FALSE;
        }
        else {

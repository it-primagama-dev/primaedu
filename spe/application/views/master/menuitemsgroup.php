			...
			<table class="table table-striped table-bordered table-hover dt-responsive" cellspacing="0" width="100%" id="tableMenu">
				<thead>
					<tr>
						...
						<th>Menu Items Group Id</th>
						<th>Menu Items Group Name</th>
						<th>Menu Items Group Order</th>
						<th>Action</th>
					</tr>
				</thead>
			</table>
				...
				<form action="javascript:void(0)" id="form" class="form-horizontal">
					...
					<div class="form-body">
						<div class="form-group">
							<div class="col-md-12">
								<label class="control-label">Menu Items Group Name</label>
								<input type="text" name="MenuItemsGroupName" id="MenuItemsGroupName" class="form-control" placeholder="Menu Items Group Name"/>
							</div>
						</div>

						<div class="form-group">
							<div class="col-md-12">
								<label class="control-label">Menu Items Group Order</label>
								<input type="number" name="MenuItemsGroupOrder" id="MenuItemsGroupOrder" class="form-control" placeholder="Menu Items Group Order"/>
							</div>
						</div>
...
$(document).ready(function() {
	jQueryTable = $("#tableMenu").DataTable({
		"ajax": {
			"url":base_url + "master/find_menuitemsgroup",
			"type":"GET",
		...
		"columns": [
			...
			{"data":"MenuItemsGroupOrder","autoWidth":true},
...
function modal_form() {
	...
	$.ajax({
		...
		success: function(response)
		{
			...
			$('[name="MenuItemsGroupName"]').val(response.data[0].MenuItemsGroupName);
			$('[name="MenuItemsGroupOrder"]').val(response.data[0].MenuItemsGroupOrder);
...
function delete_form() {
	if (confirm("Are you sure you want to delete this ?")) {
		$.ajax({
			url : base_url + "master/delete_menuitemsgroup",
			type: "POST",
...
function save()
{
	...
	if (save_method == 'edit') {
		...
		url = base_url+'master/adit_menuitemsgroup';
	} else {
		url = base_url+'master/add_menuitemsgroup';
	}
	formdata['MenuItemsGroupName'] = window.btoa(unescape(encodeURIComponent($('[name="MenuItemsGroupName"]').val())));
	formdata['MenuItemsGroupOrder'] = window.btoa(unescape(encodeURIComponent($('[name="MenuItemsGroupOrder"]').val())));

				...
				<form action="javascript:void(0)" id="form" class="form-horizontal">
					...
					<div class="form-body">
						<div class="form-group">
							<div class="col-md-12">
								<label class="control-label">Paket Program</label>
								<input type="text" name="PackName" id="PackName" class="form-control" placeholder="Menu Item."/>
							</div>
						</div>
						<div class="form-group">
							<div class="col-md-12">
								<label class="control-label">Jumlah Matpel</label>
								<select name="PackType" id="PackType" class="form-control" placeholder="Status">
...
$(document).ready(function() {
	jQueryTable = $("#tableMenu").DataTable({
		"ajax": {
			"url":base_url + "program/find_pack",
			"type":"GET",
...
function modal_form() {
	...
	$.ajax({
		...
		success: function(response)
		{
			...
			$('[name="PackType"]').val(response.data[0].PackType);
...
function delete_form() {
	if (confirm("Anda yakin akan menghapus data ini?")) {
		$.ajax({
			url : base_url + "program/delete_pack",
			type: "POST",
...
function save()
{
	...
	if (save_method == 'edit') {
		...
		url = base_url+'program/edit_pack';
	} else {
		url = base_url+'program/add_pack';
	}
	...
	formdata['PackType'] = window.btoa(unescape(encodeURIComponent($('[name="PackType"]').val())));

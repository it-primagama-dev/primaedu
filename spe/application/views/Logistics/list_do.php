...
<div class="row">
    <div class="col-lg-12">
        <div class="form-group">
            <div class="col-lg-6">
            <p style="font-size: 23px; font-weight: bold;"><i class="fa fa-list"> Input Barang yang akan di Kirim</i></p>
            </div>
            <div class="col-lg-6" style="text-align: right; display: none;" id="ubahdiv">
            <p><a href="javascript:void(0)" class="btn btn-warning" id="btnbc"><i class="fa fa-pencil"> Ubah Detail Pengiriman</i></a></p>
            </div>
        </div>
    </div>
</div>
...
function reload_data() {
    //var PR = $('#PR').val();
    $.ajax({
        url : base_url+"Logistics/get_list_do",
        type: 'GET',
        //data: ({PR:PR}),
        dataType: 'json',
        beforeSend: function(){
            ...
            if (jml_data > 0) {
                ...
                $.each(data.rows, function(i, item) {
                    ...
                    var $tr = $('<tr>').append(
                        ...
                        $('<td class="print-hidden">').html("<a href=\"javascript:void(0)\" class=\"btn btn-info btn-xs\" onclick=\"Input_DO('"+item.PR_Number+"')\"> Input Detail</a>")
                    ).appendTo('#data_po');
                });
                $('#example').dataTable();
            } else {
...
function Input_DO(pr) {
    redirectPost(base_url+'Logistics/input_do',{PR:window.btoa(pr),token:window.btoa("Q3IzNHQzZF9ieS5IQG1aNGg="),kode:window.btoa(1)})
}

$("#btnbc").click(function(e) {
    window.location.href = base_url + "Logistics/list_edit_do";
});
</script>